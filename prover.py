...
class Prover:
    ...
    def add_universal_instantiation(self, instantiation: Union[Formula, str],
        ...
        # Task 10.1
        new_instantiation = Formula('->', self._lines[line_number].formula,
                                    instantiation)
        instantiation_map = {'x': self._lines[line_number].formula.variable,
                             'R': self._lines[
                                 line_number].formula.predicate.substitute(
                                 {self._lines[
                                     line_number].formula.variable: Term.parse(
                                     '_')}),
                             'c': term}
        self.add_instantiated_assumption(new_instantiation, Prover.UI,
                                         instantiation_map)
        self.add_mp(instantiation, line_number, len(self._lines) - 1)
    ...
    def add_tautological_implication(self, implication: Union[Formula, str],
        ...
        new_formulas = [implication]
        last_formula = implication
        for number in numbers_list:
            new_formula = Formula('->', self._lines[number].formula,
                                  last_formula)
            last_formula = new_formula
            new_formulas.append(new_formula)
        last_line = self.add_tautology(new_formulas[-1])
        for i in range(len(numbers_list) - 1, -1, -1):
            last_line = self.add_mp(new_formulas[i], numbers_list[i],
                                    last_line)
        return last_line
    ...
    def add_existential_derivation(self, consequent: Union[Formula, str],
        ...
        # Task 10.3
        var = self._lines[line_number1].formula.variable
        ug_formula2 = Formula('A', var, self._lines[line_number2].formula)
        ug_line = self.add_ug(ug_formula2, line_number2)
        and_formula = Formula('&', ug_formula2,
                              self._lines[line_number1].formula)
        es_line = self.add_instantiated_assumption(
            Formula('->', and_formula, consequent),
            Prover.ES, {'x': var, 'Q': consequent,
                        'R': self._lines[line_number1].
                        formula.predicate.substitute({var: Term('_')})})
        taut_line = self.add_tautological_implication(and_formula,
                                                      {line_number1, ug_line})
        return self.add_mp(consequent, taut_line, es_line)
    ...
    def add_flipped_equality(self, flipped: Union[Formula, str],
        ...
        assert equality == Formula('=', [flipped.arguments[1],
                                         flipped.arguments[0]])
        # Task 10.6
        eq_implies = Formula('->', Formula('=', [equality.arguments[0],
                                                 equality.arguments[0]]), flipped)
        imp_line = self.add_instantiated_assumption(Formula(
            '->', equality, eq_implies), Prover.ME, {
            'R': Formula('=', [Term('_'), equality.arguments[0]]),
            'c': equality.arguments[0], 'd': equality.arguments[1]})
        mp_line = self.add_mp(eq_implies, line_number, imp_line)
        rx_line = self.add_instantiated_assumption(Formula(
            '=', [equality.arguments[0], equality.arguments[0]]),
            Prover.RX, {'c': equality.arguments[0]})
        return self.add_mp(flipped, rx_line, mp_line)
    ...
    def add_free_instantiation(self, instantiation: Union[Formula, str],
        ...
        for var, term in substitution_map.items():
            next_gen = next(fresh_variable_name_generator)
            prev_line = self.add_ug(
                Formula('A', var, self._lines[prev_line].formula), prev_line)
            prev_line = self.add_universal_instantiation(
                self._lines[prev_line].formula.predicate.substitute({var: Term(next_gen)}, {}),
                prev_line, next_gen)
            prev_line = self.add_ug(
                Formula('A', next_gen, self._lines[prev_line].formula), prev_line)
            prev_line = self.add_universal_instantiation(
                self._lines[prev_line].formula.predicate.substitute({next_gen: term}, {}),
                prev_line, term)
        return prev_line
    ...
    def add_substituted_equality(self, substituted: Union[Formula, str],
        ...
        # Task 10.8
        substituted_0 = parametrized_term.substitute(
            {'_': self._lines[line_number].formula.arguments[0]}, set())
        substituted_1 = parametrized_term.substitute(
            {'_': self._lines[line_number].formula.arguments[1]}, set())
        eq_formula = Formula('=', [substituted_1, substituted_0])
        rx_formula = Formula('=', [substituted_0, substituted_0])
        mid_imp = Formula('->', rx_formula, eq_formula)
        me_line = self.add_instantiated_assumption(Formula(
            '->', self._lines[line_number].formula, mid_imp), Prover.ME,
            {'R': Formula('=', [parametrized_term, substituted_0]), 'c': self._lines[line_number].formula.arguments[0],
             'd': self._lines[line_number].formula.arguments[1]})
        mp_line = self.add_mp(mid_imp, line_number, me_line)
        rx_line = self.add_instantiated_assumption(
            rx_formula, Prover.RX, {'c': substituted_0})
        mp_line = self.add_mp(eq_formula, rx_line, mp_line)
        return self.add_flipped_equality(substituted, mp_line)
    ...
    def _add_chaining_of_two_equalities(self, line_number1: int,
        ...
        # Task 10.9.1
        arg0 = self._lines[line_number1].formula.arguments[0]
        arg1 = self._lines[line_number1].formula.arguments[1]
        arg2 = self._lines[line_number2].formula.arguments[1]
        flipped_1 = self.add_flipped_equality(Formula('=', [arg1, arg0]), line_number1)
        new_imp = self.add_instantiated_assumption(
            Formula("->", self._lines[flipped_1].formula, Formula('->', equality2, Formula(
                '=', [arg0, arg2]))), Prover.ME,
            {'R': Formula('=', [Term('_'), arg2]), 'c': arg1, 'd': arg0})
        new_mp = self.add_mp(Formula('->', equality2, Formula('=', [arg0, arg2])), flipped_1, new_imp)
        return self.add_mp(Formula('=', [arg0, arg2]), line_number2, new_mp)
    ...
    def add_chained_equality(self, chained: Union[Formula, str],
        ...
        for i in line_numbers[1:]:
            new_line = self._add_chaining_of_two_equalities(
                new_line, i)
        return new_line
